...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get("FLASK_APP_ID")
...
class User(UserMixin, db.Model):
    ...
    comments = relationship("UserComment", back_populates="author")
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        salted_password = generate_password_hash(register_form.password.data, method='pbkdf2:sha256', salt_length=8)
...
def show_post(post_id):
    ...
    requested_comments = UserComment.query.filter_by(post_id=post_id)
    ...
    if comment_form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("post.html", post=requested_post, comments=requested_comments, form=comment_form, logged_in=current_user)
